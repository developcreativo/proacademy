            ...
            <div class="table-responsive">
                <table class="table table-bordered table-striped mb-none" id="datatable-details">
                    ...
                    @foreach($lists as $item)
                        <tr>
                            <td class="cu-p" data-toggle="modal" href="#description{{ $item->id }}">{{ $item->title }}</td>
                            <td class="text-center" width="150">{{ date('d F Y / H:i',$item->created_at) }}</td>
                            <td class="text-center" title="{{ !empty($item->requester) ? $item->requester->username : '' }}">{{ !empty($item->requester) ? $item->requester->name : '' }}</td>
